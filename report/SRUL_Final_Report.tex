...
\begin{abstract}
Autoencoders can learn latent spaces that are not Gaussian. When normalization is used, their features may concentrate near a fixed-radius shell; Transformer autoencoders with LayerNorm are one important example. Ignoring this geometry can make the generative prior move through latent regions that do not match the encoder output. We use \emph{Spherical Riemannian Unified Latents} (SRUL) to examine this problem in a controlled setting. SRUL makes spatial latent tokens exactly spherical and models them with Riemannian Flow Matching. On CIFAR-10, replacing a Euclidean chord with a geodesic path improves FID from $67.44$ to $63.84$ while keeping the encoder and decoder unchanged. We also study tangent-noise scale and classifier-free guidance, and evaluate the same approach on PathMNIST and CelebA-64.
\end{abstract}
...
Latent generative models use an autoencoder to compress an image and a prior to generate new latent codes. The geometry of these codes is learned by the encoder and does not have to be Gaussian. In particular, normalization can make feature norms nearly constant, so the latent representation becomes close to a hypersphere. Transformer autoencoders with LayerNorm are one common example, while explicit feature normalization can produce the same structure in other architectures.

This geometry should not be ignored when the prior is designed. If the encoder produces fixed-radius latents but the prior uses ordinary Euclidean interpolation, the intermediate states can leave the region represented by the encoder. Even with normalized endpoints, a straight path forms a chord through the sphere rather than moving along its surface.

The same mismatch can be stated at the distribution level. Let $q_\phi(z)$ denote the aggregate distribution of encoded latents. For any prior $p(z)$,
\begin{equation}
\mathbb E_x\!\left[\mathrm{KL}\big(q_\phi(z\mid x)\|p(z)\big)\right]
= I_q(X;Z)+\mathrm{KL}\big(q_\phi(z)\|p(z)\big).
\label{eq:kl}
\end{equation}
The first term is the image information retained by the latent. The second term is the mismatch between the encoder distribution and the chosen prior. When every token has fixed norm, a prior or transport path that introduces radial variation models states that the encoder does not normally produce.

We use SRUL as a controlled way to examine this issue. It makes the spherical latent geometry exact, then compares a chord-based prior with a geometry-consistent Riemannian prior while keeping the encoder and decoder fixed. We also study tangent-noise scale and classifier-free guidance, and evaluate the final method on CIFAR-10, PathMNIST, and CelebA-64.
