...
Autoencoders do not always produce Gaussian latent spaces. Their geometry depends on the encoder and its normalization. In particular, normalization can make latent-token norms almost constant, so the codes lie close to a hypersphere. Transformer autoencoders with LayerNorm are one common example, but the same structure can also be imposed in other encoders.

If we ignore this geometry and use a Euclidean prior, the probability path can pass through states that the encoder does not produce. SRUL gives us a controlled way to examine this issue: we keep the spherical encoder and decoder fixed, then compare a Euclidean chord with a geodesic Riemannian path.

The main question is whether geometry-matched transport improves generation.
