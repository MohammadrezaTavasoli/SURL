"""
VAE prior-geometry comparison for the SRUL project.

This experiment keeps one trained KL-regularized spatial autoencoder fixed and
compares two priors on exactly the same VAE latent distribution:

1. VAE + standard linear Flow Matching (matched VAE support)
2. VAE + projected spherical RFM (direction-only mismatch)

The second variant normalizes every sampled VAE token to a unit direction,
trains RFM on the resulting product of spheres, restores a class/token mean
radius at sampling time, and then applies the unchanged VAE decoder.

The script also measures whether the VAE uses radius and posterior variance:
- reconstruction from posterior mean,
- reconstruction from posterior sample,
- reconstruction after replacing sample-specific radius by class/token mean,
- reconstruction from unit directions only.

The intended conclusion is not assumed in advance. If the projected-RFM
variant degrades, the result supports matching the prior geometry to the
encoder support. If it remains competitive, the VAE latent is already close
to direction-dominated for this architecture.

Required helper file:
    srul_medmnist_conditional_experiment.py

Recommended CIFAR-10 checkpoint from the previous compact LDM baseline:
    .../SRUL_Final_Comparisons/CIFAR10/seed_0/checkpoints/ldm_autoencoder_final.pt
"""
