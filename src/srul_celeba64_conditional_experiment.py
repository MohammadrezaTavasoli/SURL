"""
CelebA-64 conditional SRUL experiment with spatial product-of-spheres latents.

Purpose
-------
This experiment tests whether the best SRUL design found on 32x32 datasets
scales to a structurally different and higher-resolution domain: aligned face
images at 64x64 resolution. It uses the binary CelebA ``Smiling`` attribute for
conditioning and compares:

    cond_rfm  : conditional Riemannian flow matching
    cond_srul : conditional RFM with soft Jacobi weighting

The model uses an 8x8 grid of spherical tokens. Each 32-dimensional token is
normalized independently, so the latent manifold is a product of 64 spheres.
The prior uses EMA and classifier-free guidance while keeping every sampling
step on the product-of-spheres manifold.

Dataset access
--------------
By default the code streams the official CelebA-derived ``flwrlabs/celeba``
Hugging Face dataset, materializes only the requested subset, resizes aligned
faces to 64x64, and stores the subset as memory-mapped NumPy arrays. This avoids
downloading the complete 200K-image dataset on every Colab session.

Recommended Colab run
----------------------
python srul_celeba64_conditional_experiment.py \
  --out-dir /content/drive/MyDrive/SRUL_CelebA64/conditional_v1 \
  --data-root /content/drive/MyDrive/SRUL_Datasets/CelebA64 \
  --dataset celeba64 --train-samples 30000 --test-samples 5000 \
  --ae-epochs 40 --prior-epochs 60 --batch-size 64 --amp --resume

Quick synthetic smoke test
--------------------------
python srul_celeba64_conditional_experiment.py \
  --dataset fake64 --out-dir /tmp/srul_celeba_smoke \
  --train-samples 128 --test-samples 64 \
  --ae-epochs 1 --prior-epochs 1 --batch-size 8 \
  --base-channels 16 --latent-channels 8 \
  --prior-width 32 --prior-depth 2 \
  --metric-samples 32 --pr-samples 16 \
  --methods cond_rfm cond_srul --guidance-scales 1.0 2.0 \
  --skip-heavy-metrics
"""
...
class ExperimentConfig:
    dataset: str = "celeba64"
    data_root: str = "./data"
    out_dir: str = "./SRUL_CelebA64_conditional"
    seed: int = 0
    ...
    # Hugging Face CelebA subset/cache settings.
    ...
    latent_size: int = 8  # image_size / 8 for the encoder architecture
...
class CachedCelebA64Dataset(Dataset):
    """Memory-mapped CelebA subset created from a streamed HF dataset."""

    def __init__(self, cache_dir: Path):
        metadata_path = cache_dir / "metadata.json"
        if not metadata_path.exists():
            raise FileNotFoundError(f"Incomplete CelebA cache: {cache_dir}")
        metadata = json.loads(metadata_path.read_text())
        self.cache_dir = cache_dir
        self.length = int(metadata["length"])
        self.image_size = int(metadata["image_size"])
        self.attribute = str(metadata["attribute"])
        self.images = np.load(cache_dir / "images.npy", mmap_mode="r")
        self.labels = np.load(cache_dir / "labels.npy", mmap_mode="r")
        if len(self.images) != self.length or len(self.labels) != self.length:
            raise RuntimeError(f"Corrupt CelebA cache: {cache_dir}")

    def __len__(self) -> int:
        return self.length

    def __getitem__(self, index: int) -> Tuple[torch.Tensor, int]:
        # Copy is intentional: PyTorch warns for read-only NumPy memmaps.
        array = np.array(self.images[index], copy=True)
        image = torch.from_numpy(array).float().div_(127.5).sub_(1.0)
...
def _center_crop_resize(image: Image.Image, image_size: int) -> Image.Image:
    ...
    return image.resize((image_size, image_size), resampling.BICUBIC)


def prepare_celeba64_cache(
    root: str,
    split: str,
    num_samples: int,
    seed: int,
    ...
    """Stream and materialize only the requested CelebA subset once."""
    if hf_load_dataset is None:
        raise RuntimeError(
            "Hugging Face datasets is required. In Colab run: "
            "pip install -U datasets huggingface_hub"
        )
    if num_samples <= 0:
        raise ValueError("CelebA requires a positive subset size.")

    safe_attribute = attribute.replace("/", "_").replace(" ", "_")
    cache_dir = ensure_dir(
        Path(root).expanduser()
        / "celeba64_cache"
        / f"{split}_{safe_attribute}_{image_size}px_{num_samples}_seed{seed if split == 'train' else 0}"
    )
    metadata_path = cache_dir / "metadata.json"
    images_path = cache_dir / "images.npy"
    labels_path = cache_dir / "labels.npy"

    if metadata_path.exists() and images_path.exists() and labels_path.exists():
        print(f"Using cached CelebA subset: {cache_dir}")
        return cache_dir

    print(
        f"Preparing CelebA cache: repo={repo_id}, split={split}, "
        f"samples={num_samples}, attribute={attribute}"
    )
    stream = hf_load_dataset(repo_id, split=split, streaming=True)
    if split == "train":
        stream = stream.shuffle(seed=seed, buffer_size=max(shuffle_buffer, 1000))
    stream = stream.take(num_samples)

    images_tmp = cache_dir / "images.tmp.npy"
    labels_tmp = cache_dir / "labels.tmp.npy"
    ...
    count = 0
    try:
        for count, sample in enumerate(stream, start=1):
            if "image" not in sample:
                raise KeyError("CelebA sample has no 'image' field.")
            if attribute not in sample:
                raise KeyError(
                    f"CelebA attribute '{attribute}' was not found. "
                    f"Available keys include: {list(sample.keys())[:12]}"
                )
            pil = _center_crop_resize(sample["image"], image_size)
            array = np.asarray(pil, dtype=np.uint8)
            images[count - 1] = np.transpose(array, (2, 0, 1))
            raw_label = sample[attribute]
            if isinstance(raw_label, (bool, np.bool_)):
                label = int(raw_label)
            else:
                label = int(float(raw_label) > 0)
            labels[count - 1] = label

            if count % 1000 == 0 or count == num_samples:
                print(f"  cached {count}/{num_samples} images")
    ...
    if count != num_samples:
        for path in (images_tmp, labels_tmp):
            if path.exists():
                path.unlink()
        raise RuntimeError(
            f"CelebA stream ended after {count} samples; expected {num_samples}."
        )

    os.replace(images_tmp, images_path)
    ...
    save_json(
        {
            ...
            "attribute": attribute,
            "seed": seed,
            "class_names": [f"not_{safe_attribute.lower()}", safe_attribute.lower()],
        },
        metadata_path,
    )
    print(f"CelebA cache saved to: {cache_dir}")
    return cache_dir
...
def make_datasets(
    ...
    if name == "celeba64":
        train_count = train_samples or 30000
        test_count = test_samples or 5000
        train_cache = prepare_celeba64_cache(
            root=root,
            split="train",
            ...
            image_size=image_size,
            repo_id=hf_dataset,
            attribute=celeba_attribute,
            shuffle_buffer=hf_shuffle_buffer,
        )
        test_cache = prepare_celeba64_cache(
            root=root,
            split="test",
        ...
        return CachedCelebA64Dataset(train_cache), CachedCelebA64Dataset(test_cache)
    ...
    if name == "fake64":
        ...
        train_set: Dataset = datasets.FakeData(
            size=train_size,
        ...
        test_set: Dataset = datasets.FakeData(
            size=test_size,
            image_size=(3, image_size, image_size),
            num_classes=2,
            transform=transform,
...
class SpatialEncoder(nn.Module):
    """Three stride-2 downsamples: 3x64x64 -> Cx8x8 by default."""
...
class SpatialDecoder(nn.Module):
    """Three nearest-neighbor upsampling stages: Cx8x8 -> 3x64x64."""
...
def collect_real_labels(loader: DataLoader, max_samples: int) -> torch.Tensor:
    ...
    for _, batch_labels in loader:
        ...
        batch_labels = batch_labels[:remaining].long().cpu()
        labels.append(batch_labels)
        count += batch_labels.numel()
        if count >= max_samples:
            break
    if not labels:
        raise RuntimeError("No labels were collected from the evaluation loader.")
...
def sample_flow_latents(
    ...
    """Class-balanced conditional sampling with classifier-free guidance."""
    if method not in {"cond_rfm", "cond_srul"}:
...
def run_experiment(config: ExperimentConfig) -> None:
    ...
    print("=" * 80)
    print("CelebA-64 conditional SRUL on spatial product-of-spheres latents")
    print("Device:", device)
    ...
    # Match the generated attribute distribution to the real test split.
    ...
    for method, field in fields.items():
        for guidance_scale in config.guidance_scales:
            ...
            class_counts = [
                int((sample_labels == class_id).sum().item())
                for class_id in range(config.num_classes)
            ]
            available_per_class = min(class_counts) if class_counts else 0
            grid_per_class = min(
...
def parse_args() -> ExperimentConfig:
    parser = argparse.ArgumentParser(
        description=(
            "CelebA-64 conditional SRUL with 8x8 spherical tokens, "
            "EMA, soft Jacobi weighting, and CFG."
        )
    )
    parser.add_argument("--dataset", choices=["celeba64", "fake64"], default="celeba64")
    parser.add_argument("--data-root", default="./data")
    parser.add_argument("--out-dir", default="./SRUL_CelebA64_conditional")
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "optional way to reuse a compatible CelebA-64 autoencoder."
        ),
    )
    parser.add_argument(
        "--skip-ae-training",
    ...
    parser.add_argument("--grid-samples-per-class", type=int, default=16)
    ...
    if args.image_size % 8 != 0:
        parser.error("--image-size must be divisible by 8.")
    if args.dataset == "celeba64" and args.num_classes != 2:
        parser.error("CelebA Smiling conditioning requires --num-classes 2.")
