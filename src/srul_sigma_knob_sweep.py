"""
Full sigma_enc (information-control knob) experiment for SRUL.

For each requested tangent-noise value, this script:
  1. loads one fixed trained spatial spherical autoencoder;
  2. measures reconstruction quality after tangent perturbation;
  3. trains a separate class-conditional RFM prior on the corresponding noisy
     latent distribution;
  4. evaluates generated FID/KID/precision/recall with the same architecture,
     training budget, EMA, and guidance scale.

This is the correct experiment for the claimed reconstruction--prior trade-off.
Only measuring reconstruction at several sigmas is not sufficient because the
prior must also be retrained for each target latent distribution.

The script imports the existing project module for datasets, architecture,
geometry, and metrics:
  - srul_medmnist_conditional_experiment.py for CIFAR-10/PathMNIST
  - srul_celeba64_conditional_experiment.py for CelebA-64

Recommended main-paper run: CIFAR-10 with sigma values 0.05, 0.15, 0.30.
"""
...
@torch.no_grad()
def generate(base, ae, field, labels, config: Config, device):
    outputs = []
    latents = []
    ...
    for start in range(0, labels.numel(), config.metric_batch_size):
        ...
        for index in range(config.sample_steps, 0, -1):
            ...
            raw = cfg_prediction(
                field, z, t, y, config.guidance_scale
            )
...
def run(config: Config):
    ...
    max_test = max(config.metric_samples, config.pr_samples, config.recon_metric_samples)
    ...
    for sigma in config.sigma_values:
        print("\n" + "=" * 80)
        print("Training/evaluating sigma_enc =", sigma)
        ...
        save_grid(fake, run_dir / "samples" / f"sigma_{sigma_tag(sigma)}.png")

        row: Dict[str, object] = {
            "sigma_enc": sigma,
            "seed": config.seed,
            "guidance_scale": config.guidance_scale,
            "prior_final_loss": history[-1]["loss"] if history else float("nan"),
        }
        row.update(recon)
        if not config.skip_heavy_metrics:
            n = min(config.metric_samples, real_uint8.size(0), fake.size(0))
            row.update(
                base.compute_fid_kid(
                    real_uint8[:n], fake[:n], device, config.metric_batch_size
                )
            )
    ...
    save_json({"config": asdict(config), "results": rows}, run_dir / "summary.json")
