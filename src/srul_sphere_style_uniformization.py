#!/usr/bin/env python3
"""Sphere-Encoder-style noisy-uniformization ablation for SRUL.

This script adapts the main mechanism of *Image Generation with a Sphere
Encoder* to SRUL's spatial product-of-spheres latent.  It keeps the existing
CNN autoencoder and conditional RFM prior, but fine-tunes/trains the autoencoder
with two perturbations that share one product-manifold tangent direction:

    z_large = Exp_z(alpha * u)
    z_small = Exp_z(s * alpha * u),  s ~ Uniform(0, 0.5)

The large angle alpha is jittered during training.  The losses follow the
Sphere Encoder design:

  1. pixel/perceptual reconstruction from z_small,
  2. pixel/perceptual consistency between z_large and stopgrad(z_small),
  3. latent consistency between the clean latent and the re-encoded z_large
     reconstruction.

Unlike the original global-sphere model, alpha here denotes the total geodesic
length on SRUL's product-of-spheres manifold.  The full tangent grid is
normalized to unit product-manifold norm before scaling by alpha.

The experiment reports reconstruction, generation, direct-uniform-sampling,
and marginal-uniformity diagnostics.  It does not assume that the complete
joint spatial latent becomes factorized uniform; RFM can still learn remaining
...
@dataclass
class UniformizationConfig:
    dataset: str = "cifar10"
    data_root: str = "./data"
    out_dir: str = "./SRUL_CIFAR10_sphere_uniformization"
    seed: int = 0
    ...
    # Sphere-Encoder-style product-manifold angle jitter.
    ...
    # Loss weights adapted from the original CIFAR-10 Sphere Encoder recipe.
...
def sample_sphere_style_pair(
    z: torch.Tensor,
    config: UniformizationConfig,
...
def sphere_style_objective(
    ...
    z = ae.encode(images)
    z_large, z_small, jitter = sample_sphere_style_pair(z, config)
    ...
    # branch is detached, following the stop-gradient design in Sphere Encoder.
...
def train_uniformized_autoencoder(
    ae: base.SpatialSphericalAutoencoder,
    loader: DataLoader,
    device: torch.device,
    config: UniformizationConfig,
    ...
    latest = checkpoints / "uniformized_autoencoder_latest.pt"
    final = checkpoints / "uniformized_autoencoder_final.pt"
    log_path = logs_dir / "uniformized_autoencoder.csv"
...
def make_base_config(config: UniformizationConfig) -> base.ExperimentConfig:
...
def plot_uniform_training_curves(
    ...
    if ae_rows:
        ...
        axes[0].set_title("Sphere-style autoencoder")
...
def run(config: UniformizationConfig) -> None:
    ...
    print("=" * 88)
    print("SRUL + Sphere-Encoder-style noisy uniformization ablation")
    print("Device:", device)
    ...
    ae_logs = train_uniformized_autoencoder(
        ae, train_loader, device, config, checkpoints, logs_dir
    )
    ae.load_state_dict(
        base.load_state_dict_safely(
            checkpoints / "uniformized_autoencoder_final.pt", device
        )
    )
    ae.eval()
    ...
    if baseline_results:
        ...
        comparison_rows.append(
            {
                "model": "current_srul_autoencoder",
    ...
    comparison_rows.append(
        {
            "model": "sphere_style_uniformized_autoencoder",
    ...
    if not config.ae_only:
        ...
        for guidance_scale in config.guidance_scales:
            ...
            images = base.decode_latents(ae, latents, device, config.metric_batch_size)
            safe = f"uniformized_rfm_cfg{guidance_scale:g}".replace(".", "p")
    ...
    base.save_csv(comparison_rows, run_dir / "uniformization_comparison.csv")
    summary = {
        "config": asdict(config),
...
def parse_args() -> UniformizationConfig:
    parser = argparse.ArgumentParser(
        description=(
            "Adapt Sphere Encoder noisy spherification to SRUL's "
            "product-of-spheres autoencoder and train a conditional RFM prior."
        )
    )
    parser.add_argument("--dataset", choices=["cifar10", "fake"], default="cifar10")
    parser.add_argument("--data-root", default="./data")
    parser.add_argument("--out-dir", default="./SRUL_CIFAR10_sphere_uniformization")
