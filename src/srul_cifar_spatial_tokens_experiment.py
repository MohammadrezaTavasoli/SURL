"""
SRUL CIFAR-10 experiment with a spatial product-of-spheres latent.

Why this version exists
-----------------------
The previous global-latent experiment showed that the learned priors cover
encoded latents well, but real-vs-reconstruction recall was extremely low.
That points to the autoencoder/global bottleneck as the main limitation.

This script replaces one global spherical vector with a 4x4 grid of spherical
tokens. Each spatial token is normalized independently:

    z[:, :, i, j] in S^{C-1}

The complete latent is therefore a product of spheres:

    M = (S^{C-1})^(H*W).

It compares three matched priors on the same learned latent distribution:

    chord : spherical endpoints + Euclidean chord interpolation
    rfm   : tokenwise SLERP + tangent velocity
    srul  : tokenwise SLERP + tangent velocity + Jacobi weighting

The script supports:
- staged autoencoder/prior training;
- resumable checkpoints saved directly to Google Drive;
- reconstruction MSE, PSNR, SSIM, optional LPIPS;
- reconstruction FID/KID and feature precision/recall;
- generation FID/KID and feature precision/recall;
- latent-space precision/recall;
- direct uniform-product-of-spheres baseline;
- geometry diagnostics and saved sample grids.

Recommended workflow
--------------------
1) Train/evaluate only the autoencoder first:

python srul_cifar_spatial_tokens_experiment.py \
  --out-dir /content/drive/MyDrive/SRUL_CIFAR10/spatial_v1 \
  --ae-only --ae-epochs 60 --resume --amp

2) If reconstruction recall is acceptable, run the priors:

python srul_cifar_spatial_tokens_experiment.py \
  --out-dir /content/drive/MyDrive/SRUL_CIFAR10/spatial_v1 \
  --methods chord rfm srul --ae-epochs 60 --prior-epochs 120 \
  --resume --amp

3) Quick smoke test:

python srul_cifar_spatial_tokens_experiment.py \
  --dataset fake --out-dir /tmp/srul_spatial_smoke \
  --train-samples 128 --test-samples 64 \
  --ae-epochs 1 --prior-epochs 1 --batch-size 16 \
  --base-channels 16 --latent-channels 8 \
  --prior-width 32 --prior-depth 2 \
  --metric-samples 32 --pr-samples 16 \
  --methods chord rfm srul --skip-heavy-metrics
"""
...
class ExperimentConfig:
    dataset: str = "cifar10"
    data_root: str = "./data"
    out_dir: str = "./SRUL_CIFAR10_spatial"
    seed: int = 0
    ...
    methods: Tuple[str, ...] = ("chord", "rfm", "srul")
...
class SpatialPriorBlock(nn.Module):
    def __init__(self, width: int, time_dim: int):
        super().__init__()
...
def flow_prior_objective(
    ...
    if method == "chord":
        zt = (1.0 - t4) * target + t4 * source
        target_velocity = source - target
        omega = torch.zeros(
            batch, config.latent_size, config.latent_size, device=images.device
        )
    ...
    raw_prediction = field(zt, t)
    if method in {"rfm", "srul"}:
        prediction = tangent_projection_tokens(zt, raw_prediction)
    else:
        prediction = raw_prediction

    # Mean squared error per token; then apply Jacobi weight tokenwise.
...
def train_prior(
    ...
    if config.resume and latest.exists():
        ...
        field.load_state_dict(load_state_dict_safely(final, device))
        return rows
    ...
    for epoch in range(start_epoch, config.prior_epochs + 1):
        ...
        for images, _ in loader:
            ...
            with autocast_context(device, config.amp):
                loss, stats = flow_prior_objective(method, field, ae, images, config)
    ...
    atomic_torch_save(field.state_dict(), final)
    return rows
...
def sample_flow_latents(
    ...
    for start in range(0, num_samples, batch_size):
        ...
        if method == "chord":
            # The decoder was trained only on product-of-spheres latents.
            z_for_decoder = normalize_tokens(z)
        else:
            z_for_decoder = z

        all_latents.append(z_for_decoder.cpu())
    ...
    geometry = {
        ...
        "final_projection_for_decoder": method == "chord",
    }
...
def run_experiment(config: ExperimentConfig) -> None:
    ...
    print("=" * 80)
    print("SRUL spatial product-of-spheres CIFAR-10 experiment")
    print("Device:", device)
    print("Run directory:", run_dir)
    print("Latent:", f"{config.latent_channels} x {config.latent_size} x {config.latent_size}")
    ...
    # Always reload the final state to avoid any stale optimizer checkpoint state.
    ae.load_state_dict(load_state_dict_safely(checkpoints / "autoencoder_final.pt", device))
    ae.eval()

    reconstruction_metrics, real_recon, clean_recon, noisy_recon = evaluate_reconstruction(
    ...
    # Direct uniform-product-of-spheres diagnostic.
    uniform_count = min(config.pr_samples, len(test_set))
    real_latents = collect_real_latents(
    ...
    uniform_latents = uniform_latents_4d.flatten(1)
    ...
    save_method_grid(uniform_images, samples_dir / "uniform_product_sphere.png")
    ...
    print("Uniform product-of-spheres diagnostic:")
    print(json.dumps(uniform_record, indent=2))

    if config.ae_only:
        plot_training_curves(ae_logs, {}, figures_dir / "training_curves.png")
        print("AE-only run complete. Inspect reconstruction recall/FID before priors.")
        return
    ...
    for method in config.methods:
        ...
        field.load_state_dict(
            load_state_dict_safely(checkpoints / f"{method}_final.pt", device)
        )
        field.eval()
        fields[method] = field
    ...
    for method, field in fields.items():
        print("\n" + "-" * 80)
        print("Sampling and evaluating:", method)
        ...
        images = decode_latents(ae, latents, device, config.metric_batch_size)
        save_method_grid(images, samples_dir / f"{method}.png")

        row: Dict[str, object] = {
            key: value
            for key, value in geometry.items()
            if key not in {"norm_path", "radial_path"}
        }
        ...
        geometry_records[method] = geometry
    ...
    plot_norm_paths(geometry_records, figures_dir / "sampling_norm_paths.png")
    if not config.skip_heavy_metrics:
        plot_metric_bars(generation_rows, figures_dir / "generation_metrics.png")
...
def parse_args() -> ExperimentConfig:
    parser = argparse.ArgumentParser(
        description="SRUL CIFAR-10 experiment with spatial spherical tokens."
    )
    parser.add_argument("--dataset", choices=["cifar10", "fake"], default="cifar10")
    parser.add_argument("--data-root", default="./data")
    parser.add_argument("--out-dir", default="./SRUL_CIFAR10_spatial")
    ...
    parser.add_argument(
        "--methods",
        nargs="+",
        choices=["chord", "rfm", "srul"],
        default=["chord", "rfm", "srul"],
    )
