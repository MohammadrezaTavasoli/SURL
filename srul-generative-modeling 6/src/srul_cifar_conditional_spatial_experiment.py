"""
Class-conditional SRUL-v2 on CIFAR-10 with spatial product-of-spheres latents.

This is a complete standalone extension of the spatial-token SRUL experiment.
It keeps the strong 4x4 product-of-spheres autoencoder and modifies the latent
prior in four ways:

1. Class conditioning: v_theta(z_t, t, y) models each CIFAR-10 class with the
   same shared vector field and a learned label embedding.
2. Classifier-free training: labels are replaced by a null label with a small
   probability so the same network learns conditional and unconditional fields.
3. Classifier-free guidance at sampling:

       v_cfg = v_uncond + s * (v_cond - v_uncond).

4. EMA evaluation: an exponential moving average of the prior parameters is
   used for sampling and metrics.
5. Soft Jacobi weighting for conditional SRUL:

       w_alpha = (1-alpha) + alpha * sinc^2((1-t) Omega).

   alpha=0 gives conditional RFM; alpha=1 gives the original full Jacobi
   weighting. The recommended initial value is alpha=0.25.

The script compares:

    cond_rfm  : class-conditional Riemannian flow matching, alpha=0
    cond_srul : class-conditional RFM with soft Jacobi weighting

It evaluates guidance scales such as 1.0, 1.5, and 2.0 without retraining.
Samples are class-balanced, and all results/checkpoints are written directly to
``out_dir/seed_<seed>`` so Google Drive runs are resumable.

Recommended Colab workflow
---------------------------
Reuse the already trained spatial autoencoder:

python srul_cifar_conditional_spatial_experiment.py \
  ...
  --jacobi-alpha 0.25 \
  --prior-epochs 120 --resume --amp

Quick smoke test:

python srul_cifar_conditional_spatial_experiment.py \
  --dataset fake --out-dir /tmp/srul_conditional_smoke \
  ...
  --methods cond_rfm cond_srul --guidance-scales 1.0 1.5 \
  --skip-heavy-metrics
"""
...
class ExperimentConfig:
    dataset: str = "cifar10"
    data_root: str = "./data"
    out_dir: str = "./SRUL_CIFAR10_conditional"
    seed: int = 0
...
def run_experiment(config: ExperimentConfig) -> None:
    ...
    sample_labels = balanced_class_labels(metric_count, config.num_classes)
    ...
    for method, field in fields.items():
        for guidance_scale in config.guidance_scales:
            ...
            available_per_class = metric_count // config.num_classes
            grid_per_class = min(
...
def parse_args() -> ExperimentConfig:
    parser = argparse.ArgumentParser(
        description=(
            "Class-conditional SRUL-v2 with spatial product-of-spheres "
            "latents, EMA, soft Jacobi weighting, and CFG."
        )
    )
    parser.add_argument("--dataset", choices=["cifar10", "fake"], default="cifar10")
    parser.add_argument("--data-root", default="./data")
    parser.add_argument("--out-dir", default="./SRUL_CIFAR10_conditional")
