...
SRUL is trained in two stages. First, the autoencoder learns spatial unit-norm tokens and a decoder that is stable under tangent perturbations. The perturbation scale is an explicit information-control knob. Second, RFM learns the distribution of the encoded token grids. Sampling transports spherical noise with tangent-projected velocities and exponential-map updates.
...
## 4. Tangent-noise information control
...
This changes direction without changing radius. The scale `sigma_enc` controls how much image-specific information remains in the latent: small values preserve more detail, while large values impose a stronger perturbation. It is used as a controlled proxy for retained information, not as an exact bitrate. The sweep is implemented in `src/srul_sigma_knob_sweep.py`.
...
See [`MATHEMATICAL_DETAILS.md`](MATHEMATICAL_DETAILS.md) for the perceptual and latent-consistency losses, standard Flow Matching, a direct proof that SLERP is geodesic, exponential-map norm preservation, and the Bayes motivation for classifier-free guidance.
