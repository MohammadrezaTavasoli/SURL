...
SRUL trains an autoencoder to produce a spatial grid of unit-norm tokens, then learns their distribution with Riemannian Flow Matching (RFM) on the same product-of-spheres support.
...
CIFAR-10 is used for controlled geometry, support-matching, noise, and guidance studies. PathMNIST tests histopathology textures. CelebA-64 tests a larger `64x64` setting with an `8x8` token grid.
