...
SRUL is a course research project on latent generative modeling. Autoencoders do not always produce Gaussian latent spaces: their geometry depends on the encoder, normalization, and training objective. Normalization can make latent-token norms nearly constant, placing the codes close to a hypersphere. Transformer autoencoders with LayerNorm are one common example, but the same structure can also be imposed in convolutional encoders.

Ignoring this geometry can make the prior inconsistent with the representation. A Euclidean path may pass through latent states that the encoder does not produce. SRUL provides a controlled framework for examining this mismatch by keeping a spherical encoder and decoder fixed and comparing a Euclidean chord with geodesic Riemannian transport.

The central question is:

> Does geometry-matched transport improve generation from spherical latent tokens?
...
- [Final report](report/SRUL_Final_Report.pdf)
- [Final slides](slides/SRUL_Final_5_Slides.pdf)
- [Speaker script](slides/SRUL_5min_Speaker_Script.md)
